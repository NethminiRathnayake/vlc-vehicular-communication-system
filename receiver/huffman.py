...
raw_threshold = int((voltage_threshold / 3.3) * 65535)


voltage_threshold = 0.65
bit_delay = 0.1
message_bit_length = 35
message_timeout = 15
SYNC_DELAY = 3# maximum wait for red light

# ============ VLC RECEIVE ============
def receive_message_adc():
    print("Waiting to detect the Red light...")
    ...
    while time.time() - start_time < message_timeout:
        ...
        if voltage >= voltage_threshold:
            ...
            oled_message(["Red light", "detected"])
            break
        time.sleep(0.0001)
    else:
        print("No red light detected, timeout")
        oled_message(["No red light", "detected"])
        return None
...
# ============ HUFFMAN DECODE ============
def decode_huffman(encoded, dictionary):
    decoded = ""
    temp = ""
    for bit in encoded:
        temp += bit
        if temp in dictionary:
            decoded += dictionary[temp]
            temp = ""
    return decoded
...
def main():
    while True:
        encoded = receive_message_adc()
        if encoded:
            print("\nEncoded bits:", encoded)
            uart.write("Encoded bits: " + encoded + "\n")

            decoded = decode_huffman(encoded, huffman_dictionary)
            print("Decoded message:", decoded)
            uart.write("Decoded message: " + decoded + "\n")
